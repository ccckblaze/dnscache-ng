...
var EnhanceDns = function (conf) {
        ...
        dns.resolve4 = function (domain, options, callback) {
            var options_new, callback_new;
            ...
            if (typeof options === 'object') {
                ...
                options_new = {};
                callback_new = options;
            }

            var key = 'resolve4_' + domain;
            cache.get(key, function (error, record) {
                if (record) {
                    return callback_new(error, deepCopy(record));
                }
                ...
                try {
                    backup_object.resolve4(domain, options_new, function (err, addresses) {
                        if (err) {
                            list.forEach(function (cb) { cb(err); });
                            delete callbackList[key];
                            return;
        ...
        dns.resolve6 = function (domain, options, callback) {
            var options_new, callback_new;
            ...
            cache.get(key, function (error, record) {
                ...
                try {
                    backup_object.resolve6(domain, options_new, function (err, addresses) {
                        if (err) {
                            list.forEach(function (cb) { cb(err); });
                            delete callbackList[key];
                            return;
